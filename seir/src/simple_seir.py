...
def prepare_data(case_multiplier: float = 6, include_lhd_regions: bool = False, include_flu_regions: bool = False):
    county_data = pd.read_csv("seir/cases_by_day.csv")
...
def graphic(y: np.array, time_start: int, title: str, ylabel: str = "Reported Cases"):
    plt.style.use("seaborn-whitegrid")
    x = [i for i in range(len(y))]
    plt.plot(x, y, "-g", label="Projected")
    plt.plot(x[0:time_start], y[0:time_start], ":b", label="Actual ")
    plt.legend()
    plt.title(title)
    plt.xlabel("Days since 3/3/2020")
    plt.ylabel(ylabel)
    return plt
...
def seir(
    ...
    Args:
        ...
        plot_data (bool, optional): [description]. Show rhe result plots. Defaults to False.
